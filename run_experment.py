...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Training code - MCOD')
    parser.add_argument('--temp_f', type=float, default=10, help='Tempture parameters')
    parser.add_argument('--temp_w', type=float, default=0.01, help='Tempture parameters')
    parser.add_argument('--dir', type=str, default="log", help='dir for saving output')
    ...
    data_load_fn, dataset_name, n_classes = experiments_list[args.dataset]
    result = np.zeros((5, n_classes, 3))
    auroc = np.zeros((5, n_classes))
    pr_in = np.zeros((5, n_classes))
    pr_out = np.zeros((5, n_classes))
    sys.stdout = Logger(os.path.join(args.resume, 'log.txt'))
    for i in range(5):
        for c in range(n_classes):
            result[i, c, :] = cake_experiment(data_load_fn, dataset_name, args.p, i+1, c, args)
            print(result[i, :, :])
